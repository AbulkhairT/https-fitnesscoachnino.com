      ...
      <Container className="flex h-16 items-center justify-between gap-3 lg:h-[4.25rem]">
        <Link href="/" className="flex shrink-0 items-center gap-2">
          <Image
            ...
            width={200}
            height={56}
            className="h-10 w-auto max-w-[min(190px,48vw)] sm:h-11"
            priority
            sizes="200px"
          />
        </Link>
          ...
          {mainNav.map((item) => {
            if (item.children?.length) {
              ...
              return (
                <div key={item.label} className="relative">
                  <button
                    type="button"
                    className={`flex items-center gap-1 rounded-full px-3 py-2 text-sm tracking-wide ${
                  ...
                  {openD ? (
                    <div className="absolute right-0 top-full z-50 mt-1 min-w-[14rem] rounded-xl border border-[var(--border)] bg-[var(--surface)] py-2 shadow-lg">
                      {item.href ? (
                        <Link
                          href={item.href}
            ...
            return (
              <Link
                key={item.href}
                href={item.href!}
                className={`whitespace-nowrap rounded-full px-3 py-2 text-sm tracking-wide hover:bg-black/[0.04] ${
                  pathname === item.href
                    ? "text-[var(--foreground)]"
                    : "text-[var(--muted-foreground)]"
                }`}
